...
{
    [SerializeField] float speed;//prefabからいじれます
    int howToMove;
    int howToEscape;
    ...
    float pos;
    bool isEated = false;
    GenerateInsect instantiateInsect;
    Timer timer;
    // Start is called before the first frame update
    void Start()
    {
        instantiateInsect = GameObject.Find("comeInsect").GetComponent<GenerateInsect>();
    ...
    void Update()
    {
        ...
        else
        {
            ...
            if (Vector3.Distance(this.transform.position, screenCenter) > 10f)//画面中央からworld座標で10以上離れたら消す
            {
                Destroy(this.gameObject);
            }
        }
        pos = Vector3.Distance(transform.position, screenCenter);//中央に来たか確かめるための変数
